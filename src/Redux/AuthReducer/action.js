...
const register = (payload, toast) => (dispatch) => {
  dispatch({ type: types.REGISTER_REQUEST });
  return axios
    .post("https://ErikLewis95-auth.onrender.com/auth/signup", payload)
    .then((r) => {
      console.log(r.data);
...
const login = (payload, toast) => (dispatch) => {
  ...
  dispatch({ type: types.LOGIN_REQUEST });
  return axios
    .post("https://ErikLewis95-auth.onrender.com/auth/login", payload)
    .then((r) => {
      console.log(r)
...
const profile = (payload) => (dispatch) => {
  ...
  const options = {
    method: "GET",
    url: `https://ErikLewis95-auth.onrender.com/auth/${payload.email}`,
    headers: { Authorization: `Bearer ${payload.token}` },
  };
  return axios(options)
    .then((r) => {
